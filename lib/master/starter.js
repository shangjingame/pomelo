 ...
 starter.runServers = function(app) {
  ...
  switch(condition) {
    ...
    if(!!server) {
      ...
      servers = app.get(Constants.KEYWORDS.SERVER_TYPE_MAP)[condition];
      let runCount = 0;
      for(var i=0; i<servers.length; i++) {
        this.run(app, servers[i], () => {
          ++runCount;
          if (runCount === servers.length) {
              logger.debug(`parent process ${process.pid} exit`);
              process.exit(0);
          }
        });
      }
...
starter.run = function(app, server, cb) {
  ...
  if (utils.isLocal(server.host)) {
    ...
    for(key in server) {
      ...
      options.push(util.format('%s=%s', key, server[key]).replace(/\s+/g, ''));
    }
...
function runDetachedChild(command, host, options, cb) {
  const isWindows = os.platform() === 'win32';

  if (isWindows) {
    // Windows 解决方案
    let args = [
      '/c', 'start', '"Child"', '/B', command, ...options
    ]

    console.log(args)
    return cp.spawn('cmd.exe', args, {
      shell: true,
      detached: true,
      stdio: 'inherit',
      windowsHide: true
    ...
    // Unix-like 系统解决方案
    // return spawn('nohup', [
    //   command, options, '>', '/dev/tty', '2>&1'
    // ], {
    //   shell: true,
    //   detached: true,
    //   stdio: 'ignore'
    // });
    //  return  cp.spawn(command, options);

          var   child = cp.spawn(command, options,{detached: true, stdio: 'inherit'});
...
var spawnProcess = function(command, host, options, cb) {
  ...
  if(env === Constants.RESERVED.ENV_DEV) {
    if (pomelo.app.type == Constants.RESERVED.ALL) {
        ...
        child = runDetachedChild(command, host, options, cb);
        child.unref();
       cb?.();
    }
  } else {
