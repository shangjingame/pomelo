...
function runDetachedChild(command, host, params, cb) {
  const isWindows = os.platform() === 'win32';

  if (isWindows) {
    // Windows 解决方案
    let args = [
      '/c', 'start', '"Child"', '/B', command, ...params
    ]
    ...
    // Unix-like 系统解决方案
    return cp.spawn(command, params,{detached: true, stdio: 'inherit'});
  }
}
